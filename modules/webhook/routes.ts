...
router.post(
  ...
  async function (req: Request<{}, {}, MoralisStreamResponse>, res: Response) {
    try {
      ...
      const decodedLog = decodeLog(req.body);
      ...
      if (decodedLog.name == "BidPlaced") {
        ...
        if (card) {
          const currentHighestBid = await findBid({ card: card._id, version: card.version! });
          ...
          if (currentHighestBid) {
            ...
            const formerHighestBidder = await findUser({ address: currentHighestBid.bidder });
      ...
      if (decodedLog.name == "CardListedForSale") {
        ...
        const listingPrice = parseInt(ethers.utils.formatEther(decodedLog.args.listingPrice));
        const maxNumberOfBids = parseInt(decodedLog.args.maxNumberOfBids.toString() as string);
